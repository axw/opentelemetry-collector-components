...
func newLogsPartitioner(cfg *Config, settings component.TelemetrySettings) (logsPartitioner, []string, error) {
	sortedKeys := make([]PartitionKeyConfig, len(cfg.Keys))
	copy(sortedKeys, cfg.Keys)
	sort.Slice(sortedKeys, func(i, j int) bool { return sortedKeys[i].Name < sortedKeys[j].Name })

	keyNames := make([]string, len(sortedKeys))
	...
	partitioner, err := pc.ParseValueExpressions(ottl.NewValueExpressionsGetter(expressions))
	if err != nil {
		return nil, nil, err
	}
	return partitioner, keyNames, nil
}
